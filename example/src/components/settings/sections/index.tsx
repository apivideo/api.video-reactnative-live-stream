...
const Section: React.FC<ISectionProps> = ({
	...
	return (
		<View style={styles.subSectionsContainer}>
			...
			{Object.keys((assets.sections as any)[sectionName]).map((subSectionName, i) => {
				const currentSubSection = (assets.sections as any)[sectionName][subSectionName]
				
				if (currentSubSection.type === 'list') return (
					<SectionListItem 
						key={i}
						openSection={openSection}
				...
				if (currentSubSection.type === 'slider') return (
					<SectionSliderItem
						key={i}
						subSectionName={subSectionName}
						subSection={currentSubSection}
				...
				const value = isRtmpEndpoint
					? rtmpEndpoint
					: streamKey
				const placeholder = `Enter a valid ${isRtmpEndpoint ? 'RTMP endpoint' : 'stream key'}`
				return (
					<SectionInputItem 
						key={i}
